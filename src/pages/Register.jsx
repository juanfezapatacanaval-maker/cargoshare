...
export default function Register() {
  ...
  const [showTerminosError, setShowTerminosError] = useState(false)
  ...
  function validarPaso2() {
    ...
    // Solo empresas necesitan NIT, fecha constitución y rep legal
    if (!esConductorOIndep) {
  ...
  function validarPaso3() {
    const e = {}
    if (esConductorOIndep) {
      // Conductor e independiente solo necesitan cédula
      if (!archivos.cedulaRep) e.cedulaRep = 'La cedula es obligatoria'
    } else {
  ...
  async function enviarRegistro() {
    if (!aceptaTerminos) { setShowTerminosError(true); return }
    setShowTerminosError(false)
    setCargando(true); setErrorApi('')
    try {
      const formData = new FormData()
      Object.entries(form).forEach(([k, v]) => { if (v) formData.append(k, v) })
      // Para conductor/independiente mapear correo correctamente
      if (esConductorOIndep) {
        formData.set('correoEmpresa', form.correo)
      }
  ...
  const s = {
    wrap: { minHeight: '100dvh', background: '#060E1C', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'DM Sans,sans-serif', padding: '20px' },
    ...
    grid2: { display: 'grid', gridTemplateColumns: window.innerWidth < 480 ? '1fr' : '1fr 1fr', gap: '12px' },
    ...
    <div style={s.wrap}>
      <div style={s.card}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '14px', color: '#7A8FAD', lineHeight: '1.7', marginBottom: '24px' }}>
      ...
      {showDescarga && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.85)', zIndex: 1000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', backdropFilter: 'blur(10px)' }}
          ...
          <div style={{ background: '#0C1B35', border: '1px solid rgba(255,255,255,.1)', borderRadius: '24px 24px 0 0', padding: '28px 24px 40px', width: '100%', maxWidth: '480px' }}>
            <div style={{ width: '40px', height: '4px', background: 'rgba(255,255,255,.15)', borderRadius: '2px', margin: '0 auto 20px' }} />
            <div style={{ fontSize: '22px', fontWeight: '800', fontFamily: 'Syne,sans-serif', marginBottom: '8px', color: 'white' }}>📱 Instala la app</div>
            <div style={{ fontSize: '13px', color: '#7A8FAD', marginBottom: '20px', lineHeight: 1.6 }}>Sigue estos pasos para instalar CargoShare en tu celular.</div>
            {[
              { n: '1', ic: '🌐', title: `Abre en ${isIOS ? 'Safari' : 'Chrome'}`, desc: 'cargoshare.vercel.app' },
              { n: '2', ic: isIOS ? '⬆️' : '⋮', title: isIOS ? 'Toca el botón compartir ↑' : 'Toca los 3 puntos arriba', desc: isIOS ? 'Ícono cuadrado con flecha' : 'Menú del navegador' },
              { n: '3', ic: '✅', title: 'Añadir a pantalla de inicio', desc: isIOS ? 'Busca la opción en el menú' : '"Instalar aplicación"' },
            ].map(step => (
              <div key={step.n} style={{ display: 'flex', gap: '12px', background: 'rgba(255,255,255,.04)', borderRadius: '12px', padding: '12px', marginBottom: '10px', alignItems: 'flex-start' }}>
                ...
                <div style={{ fontSize: '18px', flexShrink: 0 }}>{step.ic}</div>
                <div>
            ...
            <button onClick={() => setShowDescarga(false)} style={{ width: '100%', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.1)', color: '#7A8FAD', padding: '13px', borderRadius: '11px', fontFamily: 'DM Sans,sans-serif', fontSize: '14px', fontWeight: '600', cursor: 'pointer', marginTop: '8px' }}>
              Cerrar
            </button>
          </div>
        </div>
      ...
      <div style={s.card}>
        ...
        {paso === 1 && (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
              {tiposEmpresa.map(t => (
                <div key={t.id} onClick={() => setForm({ ...form, rol: t.id })}
                  style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '16px', borderRadius: '14px',
                    border: `2px solid ${form.rol === t.id ? (esConductorOIndep && form.rol === t.id ? '#10B981' : '#F97316') : 'rgba(255,255,255,.1)'}`,
                  ...
                  {form.rol === t.id && <div style={{ color: (t.id === 'conductor' || t.id === 'independiente') ? '#10B981' : '#F97316', fontSize: '18px' }}>✓</div>}
                </div>
              ))}
            </div>
            ...
            <button onClick={siguientePaso} style={{ ...s.btn, background: esConductorOIndep ? '#10B981' : '#F97316' }}>
              Continuar →
            </button>
        ...
        {paso === 2 && (
          <div>
            <div style={{ fontSize: '16px', fontWeight: '700', color: 'white', marginBottom: '20px' }}>
              {esConductorOIndep ? 'Tus datos personales' : 'Datos de la empresa'}
            </div>

            <div style={s.fg}>
              <label style={s.lbl}>{esConductorOIndep ? 'Nombre completo *' : 'Razon social *'}</label>
              <input style={errores.nombre ? s.inpErr : s.inp}
                placeholder={esConductorOIndep ? 'Carlos Garcia' : 'Textiles del Valle S.A.S'}
                value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value, razonSocial: e.target.value })} />
            ...
            {!esConductorOIndep && (
              <>
                <div style={s.fg}>
                  ...
                  <input style={s.inp} placeholder="Calle 13 #86-60" value={form.direccion} onChange={e => setForm({ ...form, direccion: e.target.value })} />
                </div>
                <div style={s.fg}>
                  ...
                  {form.fechaConstitucion && calcularAnos() >= 1 && (
                    <div style={{ fontSize: '11px', color: '#10B981', marginTop: '4px' }}>✅ {calcularAnos()} ano(s) de operacion</div>
                  )}
                </div>
            ...
            <div style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.07)', borderRadius: '12px', padding: '16px', marginBottom: '14px' }}>
              <div style={{ fontSize: '13px', fontWeight: '700', color: 'white', marginBottom: '12px' }}>🔐 Datos de acceso</div>
              <div style={s.fg}>
                <label style={s.lbl}>Correo electronico *</label>
                <input type="email" style={errores.correoEmpresa ? s.inpErr : s.inp}
                  placeholder={esConductorOIndep ? 'tucorreo@gmail.com' : 'contacto@tuempresa.com'}
                  value={form.correoEmpresa || form.correo}
                  onChange={e => setForm({ ...form, correoEmpresa: e.target.value, correo: e.target.value })} />
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button onClick={siguientePaso} style={{ ...s.btn, background: esConductorOIndep ? '#10B981' : '#F97316' }}>Continuar →</button>
            </div>
          </div>
        )}
        ...
        {paso === 3 && (
          <div>
            ...
            <div style={{ fontSize: '13px', color: '#7A8FAD', marginBottom: '20px' }}>
              {esConductorOIndep ? 'Solo necesitamos tu cedula de ciudadanía.' : 'Sube PDF o imagen. Maximo 10MB por archivo.'}
            </div>

            {esConductorOIndep ? (
              <div style={{ marginBottom: '14px' }}>
                <label style={s.lbl}>Cedula de ciudadania *</label>
                <label style={s.fileBox(!!archivos.cedulaRep, !!errores.cedulaRep)}>
                  <input type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display: 'none' }}
                    onChange={e => { const file = e.target.files[0]; if (file) setArchivos({ ...archivos, cedulaRep: file }) }} />
                  {archivos.cedulaRep ? (
                    <div><div style={{ fontSize: '20px', marginBottom: '4px' }}>✅</div><div style={{ fontSize: '12px', color: '#10B981', fontWeight: '600' }}>{archivos.cedulaRep.name}</div></div>
                  ) : (
                    <div><div style={{ fontSize: '28px', marginBottom: '6px' }}>🪪</div><div style={{ fontSize: '12px', color: '#7A8FAD' }}>Click para subir foto o PDF de tu cedula</div></div>
                  )}
                </label>
                {errores.cedulaRep && <div style={s.err}>⚠️ {errores.cedulaRep}</div>}
              </div>
            ) : (
              [
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '8px' }}>
              ...
              <button onClick={siguientePaso} style={{ ...s.btn, background: esConductorOIndep ? '#10B981' : '#F97316' }}>Revisar y enviar →</button>
            </div>
          </div>
        )}
        ...
        {paso === 4 && (
          <div>
            ...
            <div style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.07)', borderRadius: '14px', padding: '18px', marginBottom: '16px' }}>
              <div style={{ fontSize: '12px', color: '#7A8FAD', fontWeight: '700', textTransform: 'uppercase', marginBottom: '12px' }}>Resumen</div>
              {[
                ['Tipo', tiposEmpresa.find(t => t.id === form.rol)?.titulo],
                ['Nombre', form.nombre],
                ...(!esConductorOIndep ? [['NIT', form.nit]] : []),
                ['Ciudad', form.ciudad],
                ...(!esConductorOIndep ? [['Constitucion', `${calcularAnos()} ano(s)`]] : []),
                ['Correo', form.correoEmpresa || form.correo],
            ...
            <div onClick={() => { setAceptaTerminos(!aceptaTerminos); setShowTerminosError(false) }}
              style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', background: aceptaTerminos ? 'rgba(16,185,129,.06)' : 'rgba(255,255,255,.03)', border: `1px solid ${showTerminosError ? '#EF4444' : aceptaTerminos ? 'rgba(16,185,129,.3)' : 'rgba(255,255,255,.1)'}`, borderRadius: '12px', padding: '14px', marginBottom: '16px', cursor: 'pointer' }}>
              <div style={{ width: '20px', height: '20px', borderRadius: '5px', border: `2px solid ${aceptaTerminos ? '#10B981' : 'rgba(255,255,255,.3)'}`, background: aceptaTerminos ? '#10B981' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: '1px', transition: '.2s' }}>
                {aceptaTerminos && <span style={{ color: 'white', fontSize: '12px', fontWeight: '900' }}>✓</span>}
              </div>
              <div style={{ fontSize: '13px', color: '#B8C8DC', lineHeight: 1.5 }}>
                He leído y acepto los{' '}
                <a href="https://cargoshare.vercel.app/terminos" target="_blank" rel="noreferrer"
                ...
                de CargoShare, incluyendo el tratamiento de mis datos personales conforme a la Ley 1581 de 2012.
              </div>
            </div>
            {showTerminosError && <div style={{ ...s.err, marginBottom: '12px' }}>⚠️ Debes aceptar los Términos y Condiciones para continuar</div>}
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button onClick={enviarRegistro} disabled={cargando || !aceptaTerminos}
                style={{ ...s.btn, marginTop: '8px', background: aceptaTerminos ? (esConductorOIndep ? '#10B981' : '#F97316') : 'rgba(255,255,255,.1)', opacity: cargando ? 0.7 : 1, cursor: aceptaTerminos ? 'pointer' : 'not-allowed' }}>
