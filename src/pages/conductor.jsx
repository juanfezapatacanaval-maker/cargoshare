...
function Cronometro({ fase, tipoVehiculo, onStop }) {
  ...
  function detener() { setCorriendo(false); clearInterval(intervalo.current); onStop({ segundos, horasExtra }) }
...
function ConViaje({ viaje, conductor, token, onViajeTerminado }) {
  ...
  function abrirWaze(dir) { window.open(`https://waze.com/ul?q=${encodeURIComponent(dir)}`, '_blank') }
      ...
      {fase === 'inicio' && (
        <>
          ...
          <div style={S.card}>
            ...
            <div style={{ fontSize: '14px', color: '#C8D4E3', marginBottom: '14px', lineHeight: 1.5 }}>{viaje.direccionRecogida}</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <button onClick={() => abrirMaps(viaje.direccionRecogida)} style={{ ...S.btn('#2563EB', false), width: '100%', marginTop: 0, fontSize: '13px' }}>🗺 Google Maps</button>
      ...
      {fase === 'en_ruta' && (
        <>
          ...
          <div style={S.card}>
            ...
            <div style={{ fontSize: '14px', color: '#C8D4E3', marginBottom: '14px', lineHeight: 1.5 }}>{viaje.direccionEntrega || viaje.destino}</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <button onClick={() => abrirMaps(viaje.direccionEntrega || viaje.destino)} style={{ ...S.btn('#2563EB', false), width: '100%', marginTop: 0, fontSize: '13px' }}>🗺 Google Maps</button>
      ...
      {fase === 'terminado' && (
        <div style={{ textAlign: 'center', padding: '40px 20px' }}>
          ...
          <div style={{ ...S.card, textAlign: 'left' }}>
            ...
            {resultadoDescargue && <div style={S.row}><span style={{ fontSize: '13px', color: '#7A8FAD' }}>Tiempo de descargue</span><span style={{ fontSize: '13px', fontWeight: '600' }}>{formatTime(resultadoDescargue.segundos)}</span></div>}
            {resultadoDescargue?.horasExtra?.horas > 0 && <div style={S.row}><span style={{ fontSize: '13px', color: '#7A8FAD' }}>Extra descargue</span><span style={{ fontSize: '13px', fontWeight: '600', color: '#F97316' }}>{formatCOP(resultadoDescargue.horasExtra.costo)}</span></div>}
...
function Conductor() {
  ...
  async function cargarPerfil(t) {
    try {
      ...
      if (data.viajeActivo) { setViaje(data.viajeActivo); setEstado('con_viaje') }
      else setEstado('sin_viaje')
    ...
    <div style={S.page}>
      <div style={S.nav}>
        ...
        {conductor && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            {conductor.diasLicencia <= 60 && <span style={{ fontSize: '18px' }}>⚠️</span>}
      ...
      <div style={{ padding: '12px 20px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontSize: '12px', color: '#7A8FAD' }}>{estado === 'con_viaje' ? 'Viaje activo' : 'Sin viaje activo'}</span>
        </div>
      </div>
      {estado === 'sin_viaje' && conductor && <SinViaje conductor={conductor} historial={historial} onLogout={handleLogout} />}
      {estado === 'con_viaje' && viaje && conductor && (
        <ConViaje viaje={viaje} conductor={conductor} token={token}
          onViajeTerminado={() => { setViaje(null); setEstado('sin_viaje'); cargarPerfil(token) }} />
      )}
    </div>
  )
}
